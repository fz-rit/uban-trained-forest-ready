...
def main():
    prefix = 'isolated_temp'
    root_dir = Path(f"/home/fzhcis/mylab/data/semantic3d")
    label_dir = root_dir / prefix
    label_paths = list(label_dir.glob('*.labels'))
    if len(label_paths) != 1:
        raise ValueError(f"Expected 1 label file, found {len(label_paths)}. Please check the directory structure.")
